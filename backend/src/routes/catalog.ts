...
router.post('/:type', requireRole('admin','librarian'), asyncHandler(async (req, res) => { const table = safeTable(String(req.params.type)); const data = catalogSchema.parse(req.body); const result = table === 'categories' ? await query(`INSERT INTO ${table}(name, description) VALUES ($1,$2) RETURNING *`, [data.name, data.description || null]) : await query(`INSERT INTO ${table}(name) VALUES ($1) RETURNING *`, [data.name]); await audit({ userId: req.user!.id, action: 'created', entity: String(req.params.type).slice(0,-1), entityId: result.rows[0].id, details: data, ip: req.ip }); res.status(201).json({ data: result.rows[0] }); }));
